...
Aurora is a unified, cloud-native application hosting platform built by departments, for departments, across the Government of Canada. Part of the GC Cloud One initiative, it simplifies the operation of multi-tenant Kubernetes environments at enterprise scale—providing a consistent architectural foundation for containerized solutions so teams can focus on delivering applications rather than building and operating platforms.
...
Standing up a production-grade, security-compliant Kubernetes environment is time-consuming and demands specialized expertise. Rather than have every team design, build, and operate its own cluster and platform stack, Aurora centralizes that effort into a shared foundation—reducing duplicated work, closing security gaps, and helping the Government of Canada move forward together.

## What Aurora is

Aurora is a cloud-agnostic, self-service platform built on the Cloud Native Computing Foundation (CNCF) ecosystem. It runs on the managed Kubernetes services offered by Cloud Service Providers (CSPs)—including Azure Kubernetes Service (AKS), Amazon Elastic Kubernetes Service (EKS), Google Kubernetes Engine (GKE), and Government of Canada private cloud offerings. This model offloads control plane lifecycle management to the provider, while Aurora retains architectural control over cluster composition, networking, and platform services.
...
- **Platform runtime layer.** The managed clusters and the platform components that turn a raw Kubernetes cluster into a governed, PBMM-compliant hosting platform, providing networking, security, observability, and delivery capabilities.
...
- **A Government of Canada–aligned platform**, designed to operate within the Protected B / Medium / Medium (PBMM) classification and to satisfy the associated security controls.
...
- **A unified operational model.** Lifecycle activities—version upgrades, monitoring, backup, and policy enforcement—are managed centrally and consistently across the fleet, giving workload owners a predictable operational baseline.
...
- **Built with our users.** We shape the platform alongside the departments who use it, guided by Special Interest Groups (SIGs) in areas like architecture, networking, and security.
